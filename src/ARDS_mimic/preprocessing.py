...
def main():
    project_dir = "/data/MIMIC/mini_test"
...
def main(project_dir, critical_measurements):
    patients_list_df = pd.read_csv(os.path.join(project_dir, "patients.csv"))
    for index, row in patients_list_df.iterrows():
        if index%100 == 0:
            print(index)
        ...
        if patient is not None:
            run_PCA = False
            run_patient_preprocessing(patient, run_PCA=run_PCA, columns_to_keep=critical_measurements)
...
if __name__ == "__main__":
    ...
    critical_measurements = ["Heart Rate", "SpO2", "Respiratory Rate", "Arterial BP [Systolic]", "Arterial BP [Diastolic]",
                             "Temperature F", 'PEEP Set', 'FiO2 Set', 'Arterial PaO2']
    t = time.time()
    main(project_dir, critical_measurements)